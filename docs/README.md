# cutter2 Documentation

This directory contains comprehensive documentation for the cutter2 project.

## Active Documentation

### Architecture & Development

- **[ARCHITECTURE.md](ARCHITECTURE.md)** - System architecture and design patterns
- **[DEVELOPMENT_GUIDE.md](DEVELOPMENT_GUIDE.md)** - Setup and development workflow
- **[TESTING_GUIDE.md](TESTING_GUIDE.md)** - Testing strategy and guidelines
- **[CONTRIBUTING.md](CONTRIBUTING.md)** - Contribution guidelines

### Code Reference

- **[API_REFERENCE.md](API_REFERENCE.md)** - API documentation and reference
- **[CODEBASE_REVIEW.md](CODEBASE_REVIEW.md)** - Codebase structure and review
- **[REFACTORING_PLAN.md](REFACTORING_PLAN.md)** - Ongoing refactoring initiatives

## Archived Documentation

Historical documentation for completed features and development phases can be found in the [archive](archive/) directory:

### Completed Features

- **[archive/track-offset/](archive/track-offset/)** - Track Offset UI feature (Dec 2025)
  - Full implementation documentation
  - Design specifications
  - Test results and issue resolution

### Development Phases

- **[archive/phase-2.1/](archive/phase-2.1/)** - Internationalization (Oct 2025)
- **[archive/phase-2.2/](archive/phase-2.2/)** - Performance Optimization (Oct 2025)
- **[archive/phase-2.3/](archive/phase-2.3/)** - Logging System (Planned)

## Document Status

| Document | Status | Last Updated |
|----------|--------|--------------|
| ARCHITECTURE.md | ✅ Active | 2025-10 |
| DEVELOPMENT_GUIDE.md | ✅ Active | 2025-10 |
| TESTING_GUIDE.md | ✅ Active | 2025-10 |
| CONTRIBUTING.md | ✅ Active | 2025-10 |
| API_REFERENCE.md | ✅ Active | 2025-10 |
| CODEBASE_REVIEW.md | ✅ Active | 2025-10 |
| REFACTORING_PLAN.md | ✅ Active | 2025-10 |

## Quick Links

### For New Developers
1. Start with [ARCHITECTURE.md](ARCHITECTURE.md) to understand system design
2. Follow [DEVELOPMENT_GUIDE.md](DEVELOPMENT_GUIDE.md) to set up environment
3. Read [CONTRIBUTING.md](CONTRIBUTING.md) before making changes
4. Review [TESTING_GUIDE.md](TESTING_GUIDE.md) for testing requirements

### For Feature Implementation
1. Review [REFACTORING_PLAN.md](REFACTORING_PLAN.md) for ongoing initiatives
2. Check [archive/track-offset/](archive/track-offset/) for recent implementation example
3. Follow patterns documented in [ARCHITECTURE.md](ARCHITECTURE.md)
4. Refer to [API_REFERENCE.md](API_REFERENCE.md) for API details

### For Code Review
1. Use [CODEBASE_REVIEW.md](CODEBASE_REVIEW.md) for structure overview
2. Check [CONTRIBUTING.md](CONTRIBUTING.md) for coding standards
3. Verify tests follow [TESTING_GUIDE.md](TESTING_GUIDE.md)

## Version Information

- **Project**: cutter2
- **Current Version**: 0.8.11
- **Swift**: 6.0+
- **macOS**: 15.2+ (Sequoia or later)
- **Xcode**: 16.2+

## Recent Updates

### December 2025
- ✅ Track Offset UI feature completed and documented
- ✅ All Track Offset documentation archived to `archive/track-offset/`
- ✅ Documentation structure reorganized

### October 2025
- ✅ Phase 2.1: Internationalization completed
- ✅ Phase 2.2: Performance optimization completed

## See Also

- Main project: [../README.md](../README.md)
- License: [../LICENSE.txt](../LICENSE.txt)
- Source code: [../cutter2/](../cutter2/)
